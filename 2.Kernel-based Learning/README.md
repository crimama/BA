...
- 앞선 Case 1과 Case2는 모두 선형 모델이다. 그렇기 때문에 만약 데이터의 결정 경계가 비선형의 형태를 띄는 경우 앞선 방법들을 적용하기 어려우므로 이를 위해 kernel trick을 사용하게 된다. 
- kernel trick은 학습 데이터를 고차원으로 변형 후 고차원에서 선형 분류를 수행하는 방법을 의미한다. 
- 
